...
## Boxploty
```{r}
ggplot(kredyty_inp, aes(x = Education, y = LoanAmount)) +
  geom_boxplot()
```

```{r}
ggplot(kredyty_inp, aes(x = Gender, y = LoanAmount)) +
  geom_boxplot()
```

```{r}
ggplot(kredyty_inp, aes(x = as.factor(Credit_History), y = LoanAmount)) +
  geom_boxplot()
```

```{r}
ggplot(kredyty_inp, aes(x = Property_Area, y = LoanAmount)) +
  geom_boxplot()
```

```{r}
ggplot(kredyty_inp, aes(x = Dependents, y = LoanAmount)) +
  geom_boxplot()
```

```{r}
ggplot(kredyty_inp, aes(x = Married, y = LoanAmount)) +
  geom_boxplot()
```
...
filtered_data <- kredyty_inp %>%
    mutate(Income = log(ApplicantIncome + CoapplicantIncome))
ggplot(data = filtered_data, aes(x = Income, y = LoanAmount, color = Loan_Status)) +
    ...
    labs(
        ...
        x = "Dochód Gospodarstwa Domowego",
        y = "Kwota Pożyczki [tys. USD]",
        color = "Status Pożyczki"
    ) +
    scale_color_manual(values = c("N" = "#eb4196", "Y" = "navy")) +
    theme_minimal() +
    theme(plot.title = element_text(hjust = 0.5))
