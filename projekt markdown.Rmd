...
<p style="text-align: justify;">Test Shapiro - Wilka pokazał, że zarówno zmienna Applicant Income (dochód wnioskodawcy), Loan Amount (kwota kredytu), Total Income (łączny dochód wnioskodawcy i współwnioskodawcy) oraz zmienna Loan Amount Term (okres kredytowania) nie mają rozkładu normalnego. Dla każdego testu p-value było mniejsze od poziomu istotności alfa = 0,05 - należy więc odrzucić hipotezę zerową o normalności rozkładów tych zmiennych.</p>

```{r, fig.align='center', fig.width=10, fig.height=9}
# Porównanie dochodu całkowitego względem decyzji kredytowej
z1 <- ggbetweenstats(
  data = kredyty_inp,
  x = Loan_Status,
  y = TotalIncome,
...
ggscatterstats(
  ...
  y = LoanAmount,
  messages = FALSE,
  title = "Zależność dochodu i kwoty kredytu"
)
```
...
ggbarstats(
  ...
  y = Loan_Status,
  messages = FALSE,
  title = "Wykształcenie a decyzja kredytowa"
)
```

```{r}
# Test Levene’a
leveneTest(TotalIncome ~ Loan_Status, data = kredyty_inp)

# ANOVA klasyczna
anova_income <- aov(TotalIncome ~ Loan_Status, data = kredyty_inp)
summary(anova_income)

# Test nieparametryczny
wilcox.test(TotalIncome ~ Loan_Status, data = kredyty_inp)
```
...
conf_intervals <- exp(confint(model_logistic))
conf_intervals
...
ggplot(kredyty_inp, aes(x = Predicted_Prob, fill = Loan_Status)) +
  ...
  labs(
    title = "Rozkład przewidywanego prawdopodobieństwa decyzji",
